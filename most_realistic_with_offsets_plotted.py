# Code can take 1 to 2 hours or more to run, so to avoid automatic saving failing,
# keep session active so that plots display at the end.
...
# ===============================================================
# CELL 4 — Enhanced Median‑Scenario Visualization + Diagnostics
# ===============================================================

print("Running enhanced median‑scenario visualization…")
...
print(f"Calibration inliers: {len(cal_inliers)} → {cal_inliers}")
print(f"Calibration outliers: {len(cal_outliers)} → {cal_outliers}")

print("\n=== CALIBRATION RMS PER SENSOR ===")
for i, rms in enumerate(sensor_rms):
    print(f"Sensor {i:2d}: RMS = {rms:.4f} s")

print("\n=== OFFSET COMPARISON (TRUE vs ESTIMATED) ===")
for i in range(NUM_SENSORS):
    print(f"Sensor {i:2d}: true={true_offsets[i]:+.4f}  est={est_offsets_final[i]:+.4f}")

print("\n=== RAW vs CORRECTED ARRIVAL TIMES (first 20 sensors) ===")
for i in range(min(20, NUM_SENSORS)):
    print(f"S{i:2d}: raw={obs_raw[i]:.4f}  corr={obs_corr[i]:.4f}")

# -----------------------------------------
# Plot calibration offsets
# -----------------------------------------

plt.figure(figsize=(10,4))
plt.plot(true_offsets, 'o-', label='True offsets')
plt.plot(est_offsets_final, 'x-', label='Estimated offsets')
plt.title("Calibration Offsets: True vs Estimated")
...
# -----------------------------------------
# Plot raw vs corrected arrival times
# -----------------------------------------

plt.figure(figsize=(10,4))
plt.plot(obs_raw, 'o-', label='Raw TOA')
plt.plot(obs_corr, 'x-', label='Corrected TOA')
plt.title("Raw vs Corrected Tornado Arrival Times")
plt.xlabel("Sensor index")
...
# Error for the inliers after calibration are greater than before calibration 
# because calibration can introduce small systematic biases and/or reduce geometric coverage,
# and the tornado solver is more sensitive to bias than to raw noise.

# ==============================================
# CELL 9 — Compare Multiple Runs Side-by-Side
#          + Best vs Worst Run Comparison
#          + Calibration Offset Variation Plot
# ==============================================

print("Running multi-run comparison…")

NUM_RUNS = 5  # You can change this

results = []
offset_runs = []   # store estimated offsets for each run
true_offsets_ref = None  # store true offsets from first run
...
for r in range(NUM_RUNS):
    ...
    # Save true offsets once
    if true_offsets_ref is None:
        true_offsets_ref = true_offsets.copy()

    # Save estimated offsets for this run
    offset_runs.append(est_offsets_final.copy())

    # Compute errors
    err_b = np.sqrt((xb - true_tx)**2 + (yb - true_ty)**2)
    err_a_all = np.sqrt((xa - true_tx)**2 + (ya - true_ty)**2)
    err_a_in = np.sqrt((xi - true_tx)**2 + (yi - true_ty)**2)

    results.append({
        "run": r,
        "true": (true_tx, true_ty),
        "before": (xb, yb),
        "after_all": (xa, ya),
        "after_in": (xi, yi),
        "err_before": err_b,
        "err_after_all": err_a_all,
        "err_after_in": err_a_in,
        "num_inliers": len(cal_inliers),
        "num_outliers": len(cal_outliers)
    })
...
for res in results:
    print(f"\n--- Run {res['run']} ---")
    print(f"True tornado:             {res['true']}")
    print(f"Before calibration:       {res['before']}   Error = {res['err_before']:.2f} m")
    print(f"After calibration (all):  {res['after_all']}   Error = {res['err_after_all']:.2f} m")
    print(f"After calibration (in):   {res['after_in']}   Error = {res['err_after_in']:.2f} m")
    print(f"Inliers: {res['num_inliers']}   Outliers: {res['num_outliers']}")
...
for res in results:
    plt.scatter(*res["before"], c='blue', marker='x', s=120, alpha=0.7)
    plt.scatter(*res["after_all"], c='purple', marker='X', s=120, alpha=0.7)
    plt.scatter(*res["after_in"], c='orange', marker='D', s=120, alpha=0.7)

plt.scatter(true_tx, true_ty, c='black', marker='*', s=400, label='True tornado')

plt.title("Multiple Runs — Tornado Localization Comparison")
plt.xlabel("X (m)")
plt.ylabel("Y (m)")
plt.grid(True, linestyle=':')
plt.axis('equal')
plt.legend(["Before", "After (All)", "After (Inliers)", "True"])
plt.show()
...
labels = [f"Run {r}" for r in range(NUM_RUNS)]
err_bars_before = [res["err_before"] for res in results]
err_bars_all = [res["err_after_all"] for res in results]
err_bars_in = [res["err_after_in"] for res in results]

x = np.arange(NUM_RUNS)
width = 0.25

plt.figure(figsize=(12,5))
plt.bar(x - width, err_bars_before, width, label='Before')
plt.bar(x, err_bars_all, width, label='After (All)')
plt.bar(x + width, err_bars_in, width, label='After (Inliers)')

plt.xticks(x, labels)
plt.ylabel("Error (m)")
plt.title("Error Comparison Across Multiple Runs")
...
# Use AFTER-INLIERS error as the metric
errors = np.array(err_bars_in)
...
print("\n=== BEST RUN (After Inliers) ===")
print(f"Run {best['run']}")
print(f"True tornado:             {best['true']}")
print(f"Before calibration:       {best['before']}   Error = {best['err_before']:.2f} m")
print(f"After calibration (all):  {best['after_all']}   Error = {best['err_after_all']:.2f} m")
print(f"After calibration (in):   {best['after_in']}   Error = {best['err_after_in']:.2f} m")
print(f"Inliers: {best['num_inliers']}   Outliers: {best['num_outliers']}")

print("\n=== WORST RUN (After Inliers) ===")
print(f"Run {worst['run']}")
print(f"True tornado:             {worst['true']}")
print(f"Before calibration:       {worst['before']}   Error = {worst['err_before']:.2f} m")
print(f"After calibration (all):  {worst['after_all']}   Error = {worst['err_after_all']:.2f} m")
print(f"After calibration (in):   {worst['after_in']}   Error = {worst['err_after_in']:.2f} m")
print(f"Inliers: {worst['num_inliers']}   Outliers: {worst['num_outliers']}")
...
# Best run
plt.scatter(*best["before"], c='blue', marker='x', s=200, label='Best Before')
plt.scatter(*best["after_in"], c='green', marker='D', s=200, label='Best After (Inliers)')

# Worst run
plt.scatter(*worst["before"], c='red', marker='x', s=200, label='Worst Before')
plt.scatter(*worst["after_in"], c='orange', marker='D', s=200, label='Worst After (Inliers)')

# True tornado
plt.scatter(true_tx, true_ty, c='black', marker='*', s=400, label='True tornado')

plt.title("Best vs Worst Run — Tornado Localization")
plt.xlabel("X (m)")
plt.ylabel("Y (m)")
plt.grid(True, linestyle=':')
plt.axis('equal')
...
# ----------------------------------------------
# NEW: Calibration Offset Variation Across Runs
# ----------------------------------------------

plt.figure(figsize=(12,5))

# Plot true offsets
plt.plot(true_offsets_ref, 'k--', linewidth=2, label='True offsets')

# Plot estimated offsets for each run
for r in range(NUM_RUNS):
    plt.plot(offset_runs[r], marker='o', alpha=0.6, label=f'Run {r}')

plt.title("Calibration Offset Variation Across Runs")
plt.xlabel("Sensor index")
plt.ylabel("Offset (s)")
plt.grid(True, linestyle=':')
plt.legend()
...
print("\nBest vs worst comparison + offset variation complete.")
